...
### 中优先级
- [ ] **服务健康检查增强** ⚠️ **重要问题**
  - [ ] Controller 实现独立的周期性健康检查任务（后台 goroutine）
  - [ ] 定期（如每 10 秒）主动检查所有已注册服务的健康状态
  - [ ] 不依赖心跳机制，确保手动注册的服务也能被定期检查
  - [ ] 当前问题：手动注册的服务只在注册时检查一次，后续永远不会更新健康状态
  - [ ] 当前问题：Agent 注册的服务依赖心跳触发检查，如果心跳停止或服务故障但 Agent 正常，可能无法及时发现
- [ ] 用户认证和权限管理
- [ ] 性能指标收集（Prometheus）
- [ ] 告警和通知系统
- [ ] CLI命令行工具
