#!/bin/bash
# 智能库文件复制脚本
# 使用ldd分析二进制文件的库依赖，自动复制到容器中

set -e

# 配置
CONTAINER_NAME="plum-agent-a"
DEFAULT_BINARY_PATH="/app/data/nodeA/e34a5f89d14a74695f6b2a20d132ebff-16baf3cc/app/HelloUI"
...
show_usage() {
    ...
    echo "  $0 /app/data/nodeA/.../app/HelloUI     # 分析指定路径的HelloUI"
    echo "  $0 -c plum-agent-b ./HelloUI          # 复制到指定容器"
    echo "  $0 -d ./HelloUI                       # 仅显示需要复制的库文件"
    echo ""
    echo "默认二进制文件路径: $DEFAULT_BINARY_PATH"
}
...
main() {
    ...
    # 如果没有指定二进制文件路径，使用默认路径
    if [ -z "$binary_path" ]; then
        binary_path="$DEFAULT_BINARY_PATH"
        print_info "使用默认二进制文件路径: $binary_path"
    fi
    
    CONTAINER_NAME="$container_name"
    
    print_info "智能库文件复制脚本"
    ...
    print_info "开始分析库依赖..."
    local libs_output
    libs_output=$(analyze_dependencies "$binary_path" "$verbose")
    if [ $? -ne 0 ]; then
        print_error "无法分析库依赖"
        exit 1
    fi
