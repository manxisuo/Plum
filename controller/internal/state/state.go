...
func (s *inMemoryStore) CreateTask(name string, labels map[string]string) (string, []string) {
    s.mu.Lock()
    defer s.mu.Unlock()
    id := newID()
    s.tasks[id] = task{TaskID: id, Name: name, Labels: labels}
    return id, []string{}
}

func (s *inMemoryStore) NewInstanceID(taskID string) string {
